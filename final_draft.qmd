...
Fertility rates have been falling in many parts of the world, but the reasons behind this trend are still not fully clear. From 2014 to 2020, the rate in the U.S. has consistently decreased by 2% each year (CDC, 2024). Many explanations have been suggested, including economic challenges, changing cultural values, and shifts in lifestyle or behavior, but it is not always clear which of these factors plays a causal role in individuals' decisions about whether or not to have children.

This study examines four such factors; religiosity, heavy media use, traditional gender role attitudes, and well-being,using cross-national data from the World Values Survey and causal inference methods including propensity score weighting, Double Machine Learning, and sensitivity analysis.
...
These limitations create an opportunity for my causal research to build on their findings by moving beyond correlation to more credible causal identification. By leveraging richer measures of key constructs, incorporating a broader set of controls, and applying causal inference methods such as propensity score weighting and DoubleML, my work aims to better isolate the pathways—particularly the role of social trust—through which mediated environments shape fertility outcomes.
...
### 1. High Religiosity
#### Common Support
The propensity score analysis shows strong overlap between individuals with high and low religiosity, with scores ranging from 0.019 to 0.837. This range covers the vast majority of the sample and defines the region of common support, where valid causal comparisons can be made. Within this region, individuals share similar background characteristics, making them plausibly comparable in their likelihood of being highly religious.

By restricting estimation to this region, the analysis avoids extrapolating to extreme ends of the religiosity spectrum, where no suitable comparison group exists. The results therefore apply to individuals whose religious engagement could plausibly vary in response to social, cultural, or institutional changes.

![](plots/common_support_religion.png)

#### Overall Causal Effect
The pooled DoubleML estimate indicates that high religiosity increases the number of children by 0.402 (SE = 0.0126, p < 0.001) among individuals in the region of common support. This effect remains statistically significant after adjusting for both individual-level covariates and structural country-level factors, including GDP per capita, urbanization, and life expectancy.

While moderate in magnitude, the effect suggests that religiosity independently contributes to sustaining higher fertility even in economically advanced or urbanized settings, where fertility rates tend to decline.
...
![](data/sensitivity_results/pooled_sense_plot_religion.png)

### 2. High Media Use

#### Common Support
The propensity score distribution shows strong overlap between individuals with high and low media use, with scores ranging from 0.013 to 0.902. This range encompasses most of the sample and defines the region of common support, where valid causal comparisons can be made. Within this range, individuals have broadly similar background characteristics, making them plausibly comparable in their likelihood of being high media users.

By focusing on this region, the analysis avoids extrapolation to individuals with extremely low or extremely high media exposure, for whom no suitable comparison group exists. This ensures that the estimated effects are relevant to populations whose media use could plausibly vary due to policy, technological, or cultural changes.

![](plots/common_support_media.png)
...
Restricting the analysis to this region helps avoid bias from cases with extreme predicted probabilities of high traditional gender attitudes, where there is little or no comparable group.

![](plots/common_support_gender.png)

#### Overall Causal Effect
In the pooled DoubleML analysis, high traditional gender attitudes are associated with an increase of 0.135 children (SE = 0.0118, 95% CI: 0.112 to 0.158, p < 0.001). This positive and statistically significant effect remains after adjusting for individual demographic factors and country-level structural controls.

The size of the effect is smaller than that observed for religiosity but opposite in direction from the effect of high media use. This is consistent with the idea that traditional gender norms may reinforce family roles that encourage higher fertility.
...
![](plots/countries_gender.png)

#### Sensitivity Analysis
The pooled sensitivity analysis indicates a robustness value of 0.039, meaning that an unobserved confounder would need to explain more than 3.9% of the residual variance in both treatment and outcome to fully explain away the observed effect. This threshold is greater than the explanatory power of the strongest measured covariate, suggesting moderate robustness to omitted variable bias.
...
![](data/sensitivity_results/pooled_sense_plot_gender.png)
...
Restricting analysis to this range excludes extreme cases where very few comparable individuals exist, improving the credibility of the estimated effects.

![](plots/common_support_happy.png)

#### Overall Causal Effect
In the pooled DoubleML analysis, high well-being is associated with an increase of 0.057 children (SE = 0.0113, 95% CI: 0.035 to 0.080, p < 0.001). This effect is smaller than that for high traditional gender attitudes and high religiosity but remains statistically significant even after adjusting for a broad set of individual- and country-level controls.

The positive direction suggests that higher subjective well-being may be linked to greater family formation, possibly through increased optimism about the future, stronger social ties, or greater life satisfaction.
...
Support widths vary substantially, from 0.576 in Pakistan to 0.928 in Australia, and countries with broader common support generally yield more stable estimates. In many cases, non-significant results may reflect smaller sample sizes or limited overlap rather than the absence of a relationship.

![](plots/countries_happy.png)
...
Within-country robustness values are highest in Turkey (0.0458) and Pakistan (0.0165), where statistically significant negative effects were observed. These values imply moderate robustness in Turkey and lower robustness in Pakistan, meaning relatively modest confounders could attenuate the estimated effects.

![](data/sensitivity_results/pooled_sense_plot_happy.png)
...
From a policy perspective, these results suggest that interventions aimed at influencing fertility behavior need to consider both the economic and cultural dimensions of decision-making. In particular, strategies that engage with prevailing social values, or that buffer against potentially fertility-reducing informational environments, may be more effective than those that target economic levers alone.

# Limitations and Future Work

Even with propensity score methods and DoubleML, the results could still be influenced by unmeasured confounding—especially for treatments like high well-being and traditional gender attitudes, which have lower robustness values. In some countries, small samples and narrow common support make it harder to tell if cross-national differences reflect real heterogeneity or just noise. Because the data are cross-sectional, we also can’t confirm the direction of causality. Country-level controls like GDP per capita, urbanization, and life expectancy could themselves be mediators rather than pure confounders, which means including them might underestimate some effects. Finally, the treatment definitions, while consistent, may mask meaningful cultural variation in what religiosity, media use, gender norms, or well-being actually mean across settings.

Future work could use longitudinal or quasi-experimental designs to better establish causal direction, test richer and more context-specific measures, and use hierarchical models to capture both pooled effects and country-specific variation. Mediation analysis could also help clarify whether the effects we see operate through economic security, social capital, or attitudinal shifts, and replication with other datasets would help confirm how generalizable these results are.


See Github

Study referenced (will cite properly): <https://www.demogr.mpg.de/papers/working/wp-2006-013.pdf>